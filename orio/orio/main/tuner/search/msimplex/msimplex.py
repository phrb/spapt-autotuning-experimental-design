...
class MSimplex(orio.main.tuner.search.search.Search):
    ...
    __SEARCH_DIST = (
        "search_distance"
    )  # default: 1    #now, orio isn't parsing this at all. So it's always 1. See pparser.py
    ...
    def __init__(self, params):
        ...
        if (
            len(self.refl_coefs) != 1
            or len(self.exp_coefs) != 1
            or len(self.cont_coefs) != 1
        ):
            err(
                "msimplex: reflection, expansion, or contraction coefficient can only be one value!!!!"
            )
        ...
        if self.time_limit <= 0 and self.total_runs <= 0:
            err(
                (
                    "%s search requires either (both) the search time limit or (and) the "
    ...
    def searchBestCoord(self, startCoord=None):
        ...
        if self.use_parallel_search:
            err(
                "orio.main.tuner.search.msimplex: msimplex search does not support parallel search"
            )
        ...
        if len(self.x0) != self.total_dims:
            err(
                "orio.main.tuner.search.msimplex: initial coordiniate x0 has to match the total dimensions"
            )
        ...
        perf_costs = map(
            lambda x: x[0] if len(x) == 1 else sum(x[1:]) / (len(x) - 1), perf_costs
        )
        ...
        while True:
            ...
            if simplex[1:] == simplex[:-1]:
                while not self.localmin:
                    neighbor = self.__chooseRandomNeighbor(
                        best_coord, simplex, best_coord
                    )
                    ...
                    cost = (
                        cost[0] if len(cost) == 1 else sum(cost[1:]) / (len(cost) - 1)
                    )
                    if cost < best_perf_cost:
                        ...
                        best_perf_cost = cost
                        info(
                            "msimplex: replaces best vertex with neighbor %s after arriving at a 1 point simplex"
                            % best_coord
                        )
                        break
            ...
            refl_perf_costs = map(
                lambda x: x[0] if len(x) == 1 else sum(x[1:]) / (len(x) - 1),
                refl_perf_costs,
            )
            ...
            # if cost(reflection) < cost(worst)
                ...
                if cont_coord == refl_coord:
                    ...
                    cont_coord = self.__chooseRandomNeighbor(
                        best_coord, simplex, cont_coord
                    )
                    ...
                    cont_perf_cost = (
                        temp[0] if len(temp) == 1 else sum(temp[1:]) / (len(temp) - 1)
                    )
            ...
            if next_coord == None and next_perf_cost == None:
                ...
                ssimplex[1:] = map(
                    (
                        lambda x, y: x
                        if x != y
                        else self.__chooseRandomNeighbor(best_coord, simplex, x)
                    ),
                    ssimplex[1:],
                    simplex[1:],
                )
        ...
        info(
            "-> best simplex coordinate: %s, cost: %e"
        ...
        for vname, rhs in self.search_opts.iteritems():
            ...
            if vname == self.__SEARCH_DIST:
                if not isinstance(rhs, int) or rhs <= 0:
                    err(
                        '%s argument "%s" must be a positive integer'
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if not isinstance(n, int) or rhs < 0:
                            err(
                                '%s argument "%s" must be integers greater than or equal to 0'
                                % (self.__class__.__name__, vname)
                            )
                else:
                    err(
            ...
            # reflection coefficient
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if (
                            not isinstance(n, int) and not isinstance(n, float)
                        ) or n <= 0:
                            err(
                                '%s argument "%s" must be number(s) greater than zero'
                                % (self.__class__.__name__, vname)
                            )
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if (
                            ...
                            err(
                                '%s argument "%s" must be number(s) greater than one'
                                % (self.__class__.__name__, vname)
                            )
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if (
                            ...
                            err(
                                '%s argument "%s" must be number(s) between zero and one'
                                % (self.__class__.__name__, vname)
                            )
            ...
            # shrinkage coefficient
                ...
                if (
                    (not isinstance(rhs, int) and not isinstance(rhs, float))
                    or rhs <= 0
                    or rhs >= 1
                ):
                    err(
                        '%s argument "%s" must be a single number between zero and one'
                        % (self.__class__.__name__, vname)
                    )
            ...
            # unrecognized algorithm-specific argument
                ...
                err(
                    'unrecognized %s algorithm-specific argument: "%s"'
                    % (self.__class__.__name__, vname)
                )
    ...
    def __chooseRandomNeighbor(self, bestVertex, simplex, coord):
        ...
        while len(self.used_neighbors) - 3 ** self.total_dims < 0:
            ...
            if self.total_runs > 0 and self.runs >= self.total_runs:
                info(
                    "msimplex: total runs limit reached while choosing a random neighbor"
                )
                self.breakFlag = True
                return coord
    ...
    def __checkSearchSpace(self):
        ...
        if self.space_size < self.__simplex_size:
            err(
                (
                    "the search space is too small for %s algorithm. "
                    + "please use the exhaustive search."
                )
                % self.__class__.__name__
            )
    ...
    def __getReflection(self, coord, centroid):
        ...
        return map(
            lambda x: self.addCoords(centroid, map(lambda y: x * y, sub_coord)),
            self.refl_coefs,
        )
    ...
    def __getShrinkage(self, coord, rest_coords):
        ...
        return map(
            lambda x: self.addCoords(
                coord, map(lambda y: self.shri_coef * y, self.subCoords(x, coord))
            ),
            rest_coords,
        )
