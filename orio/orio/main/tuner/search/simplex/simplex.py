...
class Simplex(orio.main.tuner.search.search.Search):
    ...
    def __init__(self, params):
        ...
        if self.time_limit <= 0 and self.total_runs <= 0:
            err(
                (
                    "orio.main.tuner.search.simplex.simplex:  %s search requires either (both) the search time limit or (and) the "
    ...
    def searchBestCoord(self, startCoord=None):
        ...
        if len(self.x0) != self.total_dims:
            err(
                "orio.main.tuner.search.simplex: initial coordiniate x0 has to match the total dimensions"
            )
        ...
        if self.use_parallel_search:
            err(
                "orio.main.tuner.search.simplex: simplex search does not support parallel search"
            )
        ...
        while True:
            ...
            if self.time_limit <= 0 or (time.time() - start_time) <= self.time_limit:
                ...
                (best_simplex_coord, best_simplex_perf_cost) = self.searchBestNeighbor(
                    best_simplex_coord, self.local_distance
                )

                # if the neighboring coordinate has a better performance cost
                if best_simplex_perf_cost < old_best_simplex_perf_cost:
                    info(
                        "---> better neighbor found: %s, cost: %e"
                        % (best_simplex_coord, best_simplex_perf_cost)
                    )
                else:
                    best_simplex_coord = simplex[0]
            ...
            if best_simplex_perf_cost < best_global_perf_cost:
                ...
                info(
                    ">>>> best coordinate found: %s, cost: %e"
        ...
        for vname, rhs in self.search_opts.iteritems():
            ...
            if vname == self.__LOCAL_DIST:
                if not isinstance(rhs, int) or rhs < 0:
                    err(
                        'orio.main.tuner.search.simplex.simplex: %s argument "%s" must be a positive integer or zero'
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if (
                            ...
                            err(
                                'man.tuner.search.simplex.simplex: %s argument "%s" must be number(s) greater than zero'
                                % (self.__class__.__name__, vname)
                            )
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if (
                            ...
                            err(
                                'man.tuner.search.simplex.simplex: %s argument "%s" must be number(s) greater than one'
                                % (self.__class__.__name__, vname)
                            )
                ...
                if isinstance(rhs, list):
                    for n in rhs:
                        if (
                            ...
                            err(
                                'man.tuner.search.simplex.simplex: %s argument "%s" must be number(s) between zero and one'
                                % (self.__class__.__name__, vname)
                            )
            ...
            # shrinkage coefficient
                ...
                if (
                    ...
                    err(
                        'man.tuner.search.simplex.simplex: %s argument "%s" must be a single number between zero and one'
                        % (self.__class__.__name__, vname)
                    )
            ...
            # unrecognized algorithm-specific argument
                ...
                err(
                    'man.tuner.search.simplex.simplex: unrecognized %s algorithm-specific argument: "%s"'
                    % (self.__class__.__name__, vname)
                )
    ...
    def __checkSearchSpace(self):
        ...
        if self.space_size < self.__simplex_size:
            err(
                (
                    "orio.main.tuner.search.simplex.simplex:  the search space is too small for %s algorithm. "
    ...
    def __getReflection(self, coord, centroid):
        ...
        return map(
            lambda x: self.addCoords(centroid, self.mulCoords(x, sub_coord)),
            self.refl_coefs,
        )
    ...
    def __getShrinkage(self, coord, rest_coords):
        ...
        return map(
            lambda x: self.addCoords(
                coord, self.mulCoords(self.shri_coef, self.subCoords(x, coord))
            ),
            rest_coords,
        )
